...
const handler = async (m, { conn, args }) => {
  try {
    if (!args[0]) throw '*⚠️ Por favor, ingresa una descripción para crear tu canción.*\n\n*Ejemplo:* /musicaia canción de amor | pop, romántico';

    const input = args.join(' ');
    const [prompt, customTags] = input.split('|').map(part => part.trim());

    if (!prompt) throw '*⚠️ Por favor, ingresa una descripción para crear tu canción.*';

    m.reply("*⏳ ¡Espera un momento! KARBOT-MD está componiendo tu canción. Esto puede tardar unos minutos...*");
    ...
    const tags = {
      title: song.title,
      artist: 'IA Musical by KARBOT-MD',
      album: 'Generado por KARBOT-MD',
      APIC: {
        mime: 'image/png',
        type: {
          id: 3,
    ...
    const tmpDir = tmpdir();
    const audioPath = join(tmpDir, `${sanitizeFileName(song.title.substring(0, 64))}.mp3`);
    ...
    m.reply(`❌ Ocurrió un error al generar la canción. Inténtalo de nuevo más tarde.`);
  }
};
