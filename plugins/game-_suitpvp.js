...
handler.before = async function(m) {
  ...
  if (room) {
    ...
    if (m.sender == room.p2 && /^(acc(ept)?|terima|aceptar|gas|aceptare?|nao|gamau|rechazar|ga(k.)?bisa)/i.test(m.text) && m.isGroup && room.status == 'wait') {
      ...
      const textplay = `🎮 *JUEGO INICIADO*\n*Jugadores:* @${room.p.split('@')[0]} 🆚 @${room.p2.split('@')[0]}\n\n💡 *Para jugar usa:* wa.me/${conn.user.jid.split('@')[0]}`;
      m.reply(textplay, m.chat, {mentions: this.parseMention(textplay)});

      const comienzop = `🎯 *TURNO DEL JUGADOR 1*\n
✂️ *Opciones:* tijera | piedra | papel\n
🏆 *Premio:* +${room.poin} XP\n
💥 *Penalización:* -${room.poin_lose} XP\n
⏰ *Tienes 30 segundos para elegir*`;

      const comienzop2 = `🎯 *TURNO DEL JUGADOR 2*\n
✂️ *Opciones:* tijera | piedra | papel\n
🏆 *Premio:* +${room.poin} XP\n
💥 *Penalización:* -${room.poin_lose} XP\n
⏰ *Tienes 30 segundos para elegir*`;
      ...
      room.waktu_milih = setTimeout(() => {
        const iniciativa = `⏰ *TIEMPO AGOTADO*\nEl juego ha sido cancelado por inactividad`;
        if (!room.pilih && !room.pilih2) this.sendMessage(m.chat, {text: iniciativa}, {quoted: m});
        else if (!room.pilih || !room.pilih2) {
          win = !room.pilih ? room.p2 : room.p;
          const textnull = `*[❗] @${(room.pilih ? room.p2 : room.p).split('@')[0]} NO ELIGIÓ A TIEMPO*`;
    ...
    if (jwb && reg.test(m.text) && !room.pilih && !m.isGroup) {
      ...
      room.text = m.text;
      m.reply(`✅ *HAS ELEGIDO ${m.text.toUpperCase()}*\nRegresa al grupo y ${room.pilih2 ? 'revisa los resultados*' : 'espera los resultados*'}`);
      if (!room.pilih2) this.reply(room.p2, '⚡ *TU OPONENTE YA ELIGIÓ, ES TU TURNO DE ELEGIR!!*', 0);
    }

    if (jwb2 && reg.test(m.text) && !room.pilih2 && !m.isGroup) {
      room.pilih2 = reg.exec(m.text.toLowerCase())[0];
      room.text2 = m.text;
      m.reply(`✅ *HAS ELEGIDO ${m.text.toUpperCase()}*\nRegresa al grupo y ${room.pilih ? 'revisa los resultados*' : 'espera los resultados*'}`);
      if (!room.pilih) this.reply(room.p, '⚡ *TU OPONENTE YA ELIGIÓ, ES TU TURNO DE ELEGIR!!*', 0);
    }
