...
const handler = async (m, { conn, text, usedPrefix, command }) => {
  if (!text) {
    ...
    await conn.sendMessage(
      m.chat,
      {
        text: `> 🜸 *agrega un enlace* » ingresa un enlace de x/twitter\n> 🜸 *ejemplo* » ${usedPrefix + command} https://twitter.com/usuario/status/123456789`,
      },
      { quoted: m }
    );
    return;
  ...
  try {
    ...
    if (res?.result.type == "video") {
      const caption = res?.result.caption
        ? `> 🜸 *descarga exitosa* » ${res.result.caption}`
        : "> 🜸 *descarga exitosa* » 🫡";
      ...
      for (let i = 0; i < res.result.media.length; i++) {
        ...
        await conn.sendMessage(
          m.chat,
          { image: { url: res.result.media[i].url }, caption: caption },
          { quoted: m }
        );
      }
      // reacción de éxito final
    ...
    await conn.sendMessage(
      m.chat,
      {
        text: `> 🜸 *error* » no se pudo descargar el contenido\n> 🜸 *solución* » verifica que el enlace sea válido`,
      },
      { quoted: m }
    );
  }
...
const TwitterDL = async (url) => {
  return new Promise(async (resolve, reject) => {
    ...
    if (!id)
      return resolve({
        status: "error",
        message: "> 🜸 *error* » url de twitter no válida",
      });
    ...
    if (response.data.code !== 200) {
      return resolve({
        status: "error",
        message: "> 🜸 *error* » no se pudo obtener el contenido del tweet",
      });
    }
