...
let handler = async (m, { args, conn, text, usedPrefix, command }) => {
    ...
    try {
        ...
        if (links.length === 0) {
            await conn.sendMessage(m.chat, { text: '*[ ❌ ] No se encontraron enlaces de descarga.*' }, { quoted: m });
            return;
        }
        ...
        if (!download) {
            await conn.sendMessage(m.chat, { text: '*[ ❌ ] Error al obtener el enlace de descarga.*' }, { quoted: m });
            return;
        }
        ...
        // Reacción de error en caso de excepción
        await conn.sendMessage(m.chat, { react: { text: '❌', key: m.key } });
