...
async function processContextInfo(contextInfo, groupChatId, lidResolver) {
  ...
  const connectionOptions = {
    ...
    mobile: MethodMobile,
    browser: opcion === '1' ? ['KARBOT-MD', 'Safari', '3.0.0'] : methodCodeQR ? ['KARBOT-MD', 'Safari', '3.0.0'] : ['Ubuntu', 'Chrome', '110.0.5481.100'],
    auth: {
      creds: state.creds,
  ...
  // MEJORA: Sistema de reconexión automática mejorado
  ...
  async function attemptReconnection() {
    ...
    reconnectAttempts++;
    console.log(chalk.yellow(`🔄 Intentando reconexión ${reconnectAttempts}/${maxReconnectAttempts}...`));

    try {
      await new Promise(resolve => setTimeout(resolve, reconnectDelay));
      await global.reloadHandler(true);
      console.log(chalk.green('✅ Reconexión exitosa.'));
      reconnectAttempts = 0; // Resetear contador en éxito
    } catch (error) {
      console.log(chalk.red(`❌ Error en reconexión ${reconnectAttempts}:`), error.message);
    ...
    if (opcion === '2' || methodCode) {
      ...
      if (!conn.authState.creds.registered) {
        ...
        setTimeout(async () => {
          ...
          console.log(chalk.yellow('🤖 KARBOT-MD - CÓDIGO DE EMPAREJAMIENTO'));
          console.log(chalk.green('══════════════════════════════════════'));
          console.log(chalk.white('📱 Introduce este código en WhatsApp:'));
          console.log(chalk.cyan.bold('┌' + '─'.repeat(28) + '┐'));
          console.log(chalk.cyan.bold('│ ') + chalk.white.bgCyan.bold(` ${codigo} `) + chalk.cyan.bold(' │'));
          console.log(chalk.cyan.bold('└' + '─'.repeat(28) + '┘'));
          console.log(chalk.green('✅ El código expira en 30 segundos'));
          console.log(chalk.green('══════════════════════════════════════'));
        }, 3000);
      }
    }
  ...
  conn.isInit = false;
  conn.well = false;
  conn.logger.info(`[　ℹ️　] Iniciando KARBOT-MD...\n`);
  ...
  function purgeSession() {
    let prekey = [];
    let directorio = readdirSync("./KarbotSession");
    let filesFolderPreKeys = directorio.filter(file => file.startsWith('pre-key-'));
    prekey = [...prekey, ...filesFolderPreKeys];
    filesFolderPreKeys.forEach(files => unlinkSync(`./KarbotSession/${files}`));
  }
  ...
  function purgeOldFiles() {
    const directories = ['./KarbotSession/', './jadibts/'];
  ...
  async function connectionUpdate(update) {
    ...
    // MEJORA: Manejo mejorado de reconexión
    ...
    // MEJORA: Manejo mejorado del QR
    if (qr) {
      if (opcion == '1' || methodCodeQR) {
        if (!qrAlreadyShown) {
          console.log(chalk.green('══════════════════════════════════════'));
          console.log(chalk.yellow('🤖 KARBOT-MD - ESCANEA EL CÓDIGO QR'));
          console.log(chalk.green('══════════════════════════════════════'));
          console.log(chalk.white('📱 Abre WhatsApp > Dispositivos conectados'));
          console.log(chalk.white('📷 Escanea el código QR que aparece arriba'));
          console.log(chalk.green('✅ La conexión se establecerá automáticamente'));
          console.log(chalk.green('══════════════════════════════════════'));
          qrAlreadyShown = true;
        }
    ...
    if (connection == 'open') {
      console.log(chalk.green('══════════════════════════════════════'));
      console.log(chalk.green('✅ KARBOT-MD CONECTADO CORRECTAMENTE'));
      console.log(chalk.green('══════════════════════════════════════'));
      console.log(chalk.white(`👤 Usuario: ${conn.user?.name || 'No disponible'}`));
      console.log(chalk.white(`📱 Número: ${conn.user?.id?.split(':')[0] || 'No disponible'}`));
      console.log(chalk.white(`🕐 Conectado: ${new Date().toLocaleString()}`));
      console.log(chalk.green('══════════════════════════════════════'));
      ...
      if (!global.subBotsInitialized) {
        ...
        try {
          ...
          console.log(chalk.green('✅ Sub-bots inicializados correctamente'));
        } catch (error) {
    ...
    if (reason == 405) {
      await fs.unlinkSync("./KarbotSession/" + "creds.json");
      console.log(chalk.bold.redBright(`[ ⚠️ ] Conexión replazada, reiniciando...`));
      process.send('reset');
    }
    ...
    if (connection === 'close') {
      if (reason === DisconnectReason.badSession) {
        ...
        if (shouldLogError('connectionReplaced')) {
          conn.logger.error(`[ ⚠️ ] Conexión reemplazada, se ha abierto otra nueva sesión.`);
        }
        await attemptReconnection();
      } else if (reason === DisconnectReason.loggedOut) {
        if (shouldLogError('loggedOut')) {
          conn.logger.error(`[ ⚠️ ] Sesión cerrada, por favor elimina la carpeta ${global.authFile} y escanea nuevamente.`);
        }
...
${Object.entries(lidDataManager.getUsersByCountry())
  ...
  setInterval(async () => {
    ...
    const bio = `• Activo: ${uptime} | KARBOT-MD`;
